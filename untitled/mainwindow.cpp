...
void MainWindow::readData()
{
    if (port->bytesAvailable() >= 2 ){
        ...
        int value = (unsigned char) data_recieved.at(0) * 16 + (unsigned char) data_recieved.at(1) * 1/16;
        unsigned char ost = (unsigned char)data_recieved.at(1) * 16;
        ost = ost * 1/16;
        qDebug() << ost;
        qDebug() << value;
        float value_new = value + (float) ost/16;
        m_console->putData(value_new, *Graph);
    }
}
